...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    if (loaded && ipDraft !== ipSaved.current) return;
    setIpDraft(jetsonIp);
    ipSaved.current = jetsonIp;
  }, [loaded, jetsonIp]);

  function saveIp() {
    const trimmed = ipDraft.trim();
    if (trimmed !== jetsonIp) {
      update({ jetsonIp: trimmed });
      ipSaved.current = trimmed;
    }
  }
...
const styles = StyleSheet.create({
  ...
  langOptionCaptionActive: {
    fontSize: 12,
    color: Colors.primary.DEFAULT,
    fontWeight: '500',
  },
  langOptionTextDisabled: {
    fontSize: 17,
    fontWeight: '700',
    color: Colors.outline.DEFAULT,
  },
  langOptionCaptionDisabled: {
    fontSize: 12,
    color: Colors.outline.DEFAULT,
  },
  soonPill: {
